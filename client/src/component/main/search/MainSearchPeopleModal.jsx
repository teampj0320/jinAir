...
export default function MainSearchPeopleModal({setPeopleModal,setAdultNum,setPediatricNum,setBabyNum, 
    ...
    useEffect(()=>{
        setTotal(adultNum+pediatricNum+babyNum);
        handleAll();
    ...
    const decrease = (type) => {
        if(handleAll){
            if(type === 'a'){
                (adultNum <= 1) ? alert('최소 인원은 1명입니다.') : setAdultNum(adultNum-1); 
            }else if(type === 'p'){
                (pediatricNum <= 0) ? alert('최소 인원은 0명입니다.') : setPediatricNum(pediatricNum-1);
            }else if(type === 'b'){
                (babyNum <= 0) ? alert('최소 인원은 0명입니다.') : setBabyNum(babyNum-1); 
            }
        }
    }
    const increase = (type) => {
        if(handleAll){
            if(type === 'a'){
                (adultNum >= 9) ? alert('최대 인원은 9명입니다.') : setAdultNum(adultNum+1); 
            }else if(type === 'p'){
                (pediatricNum >= 9) ? alert('최대 인원은 9명입니다.') : setPediatricNum(pediatricNum+1);
            }else if(type === 'b'){
                (babyNum >= 9) ? alert('최대 인원은 9명입니다.') : setBabyNum(babyNum+1); 
            }
        }
    }
