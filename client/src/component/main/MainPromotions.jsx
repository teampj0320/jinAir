            ...
            <div className='main_home_promotion_warp'>
                        ...
                        {promotion && promotion.map((item, i) => (
                            <SwiperSlide key={i}>
                                <div className='promotion_slider'>
                                    <img src={item.images} alt="" style={{borderRadius:'10px'}} />
                                    <div style={{lineHeight :'28px', textAlign:'left', paddingTop:'5px'}}>

                                        <p style={{fontSize:'16px'}}>{item.Departure_location}({item.D_acode})</p>
                                        <p  style={{fontSize:'20px', fontWeight:'600'}}>{item.Arrive_location}({item.A_acode})</p>
                                        <div style={{paddingTop:'5px'}}>
                                            <span style={{fontSize:'14px', fontWeight:'600'}}>KRW</span>
                                            <span style={{fontSize:'20px', fontWeight:'600'}} >{item.basic_price.toLocaleString()}원</span>
                                        </div>
                                        <p>{dayjs(item.Departure_date).format('YYYY.MM.DD (ddd)')}</p>
                                    </div>
                                </div>
                            </SwiperSlide>
                        ))}
                ...
                <div className='main_promotion_more_warp'>
                    <div className='main_promotion_more'>
                        <span style={{cursor:'pointer'}} onClick={() => { navigate('./customticket') }}><IoTicketSharp />맞춤 항공권 더보기</span>
                    </div>
