...
export default function Cheap() {
    ...
    const ScheduleCheckRound = () => { // 왕복
        if (vali()) {
            axios.post('http://13.209.74.163:9000/chatbot/searchSchedule', { start, end, date, endDate })
                .then(res => {
                    if (res.data.result === 1) {
    ...
    const exchangeCountryOneWay = () => {
        setStart(end);
        setEnd(start);
        axios.post('http://13.209.74.163:9000/chatbot/searchMonthCheap', { start, end, date })
            .then(res => {
                // if (res.data.result === 1) {
