...
export default function Airplane() {
    ...
    const handleAirNum = () => {
        if (validateAirnum()) {
            // 서버에서 있는지 조회
            axios.post('http://13.209.74.163:9000/chatbot/searchAirplane', { airNumber, airNumStartDate })
                .then((res) => {
                    if (res.data.result === 1) {
                        setAirNumClick(true);
                        setNoneAirNumClick(false);
                        axios.post('http://13.209.74.163:9000/chatbot/getSchedule', { airNumber, airNumStartDate })
    ...
    const handleCountry = () => {
        if (validateCountry()) {
            axios.post('http://13.209.74.163:9000/chatbot/searchSchedule', { start, end, date })
                .then(res => {
                    if (res.data.result === 1) {
                        setCountryClick(true);
                        setNoneCountryClick(false);
                        axios.post('http://13.209.74.163:9000/chatbot/getSchedule', { start, end, date })
                            .then(res =>
