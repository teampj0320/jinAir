...
export default function AdminQnaComment() {
    ...
    useEffect(() => {
        axios.post('http://13.209.74.163:9000/chatbot/detail', { "qid": qid })
            .then(res => {
                setFormData(res.data);

            })
            .catch(err => console.log(err));
        axios.post('http://13.209.74.163:9000/chatbot/getCustomerInfo', { id })
            .then(res => {
    ...
    const handleComment = () => {
        if(validate()){
            axios.post('http://13.209.74.163:9000/chatbot/updateComment', { "no": formData.no, inputData })
            .then(res => {
