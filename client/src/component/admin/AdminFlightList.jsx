...
export default function AdminAirRegister() {
  ...
  useEffect(()=>{
    axios.post('http://13.209.74.163:9000/admin/flight')
  ...
  const handleSearch = async() => {
    if (searchType === 'default' || !searchKeyword.trim()) {
      try {
        const res = await axios.post('http://13.209.74.163:9000/admin/flight');
    ...
    try {
      const res = await axios.post('http://13.209.74.163:9000/admin/flight/search', {
        type: searchType,
  ...
  const handleDelete = async() =>{
    ...
    try {
      const result = await axios.post('http://13.209.74.163:9000/admin/deleteFlightList', {fnums:selectedRows});
      if(result.data !== 0){
        alert('삭제가 완료되었습니다.');
        const res = await axios.post('http://13.209.74.163:9000/admin/flight');
        setFormData(res.data);       
