...
export default function AdminNotice() {
  ...
  useEffect(()=>{
    axios.post('http://13.209.74.163:9000/admin/noticeList')
  ...
  const handleSearch = async() =>{
    try {
      const result = await axios.post('http://13.209.74.163:9000/admin/noticeSearch', {keyword:searchKeyword.trim()});
  ...
  const handleDelete = async() =>{
    ...
    try {
      const result = await axios.post('http://13.209.74.163:9000/admin/notice/delete',{nums:selectedRows});
      if(result.data !== 0){
        alert('삭제가 완료되었습니다.');
        const res = await axios.post('http://13.209.74.163:9000/admin/noticeList');
